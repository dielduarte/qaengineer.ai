...
function createProjectDirectory(
  ...
  try {
    if (!fs.existsSync(qaEngineerDir)) {
      ...
      log.info(
        `Directory already exists: ${qaEngineerDir}`,
      );
    }
...
function createConfigurationFiles(
  ...
  try {
    ...
    log.success(
      `Created configuration file: ${configFilePath}`,
    );
    ...
    log.success(
      `Created test example file: ${testExampleFilePath}`,
    );
    return true;
  } catch (error) {
...
function updatePackageJson(projectRoot: string): boolean {
  ...
  try {
    if (fs.existsSync(packageJsonPath)) {
      ...
      log.success(
        `Updated package.json with qaengineer script.`,
      );
      return true;
    } else {
      log.warn(
        'package.json not found. Skipping script addition.',
      );
      return true;
    }
  } catch (error) {
...
function installDependencies(): boolean {
  try {
    ...
    execSync('npm install @qaengineer', {
      stdio: 'inherit',
    });
    log.success(
      'Dependency @qaengineer installed successfully.',
    );
    return true;
  } catch (installError) {
...
export async function init(
  ...
  try {
    ...
    const qaEngineerDir =
      createProjectDirectory(projectRoot);
