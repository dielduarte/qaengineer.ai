  ...
  return (
    <div className="w-full max-w-md mx-auto">
      ...
      <ul className="space-y-2">
        {todos.map(todo => (
          <li key={todo.id} className="flex items-center gap-2 bg-background border rounded px-3 py-2">
            {editingId === todo.id ? (
              <>
                <Input
                  value={editingValue}
                  ...
                  autoFocus
                />
                <Button size="sm" onClick={() => handleEditSave(todo.id)} className="px-2">Save</Button>
                <Button size="sm" variant="secondary" onClick={handleEditCancel} className="px-2">Cancel</Button>
              </>
            ) : (
              <>
                <span className="flex-1 text-left">{todo.text}</span>
                <Button size="sm" variant="outline" onClick={() => handleEdit(todo.id, todo.text)} className="px-2">Edit</Button>
                <Button size="sm" variant="destructive" onClick={() => handleRemove(todo.id)} className="px-2">Remove</Button>
              </>
            )}
          </li>
        ))}
